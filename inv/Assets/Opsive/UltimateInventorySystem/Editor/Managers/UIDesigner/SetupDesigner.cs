...
namespace Opsive.UltimateInventorySystem.Editor.Managers.UIDesigner
{
    ...
    public class UIDesignerSchemaSetupSubMenu : UIDesignerBoxBase
    {
        ...
        public void Refresh()
        {
            ...
            if (editorResult.validationState == UIDesignerSchema.ValidationState.HasErrors) {
            ...
            if (editorResult.validationState == UIDesignerSchema.ValidationState.HasWarnings) {
                m_HelpBox.SetMessage($"The selected schema '{schema.name}' has warnings:\n" +
                                     editorResult.message);
                m_ValidateResultContainer.Add(m_HelpBox);
            }
        ...
        private void FullUISetup()
        {
            ...
            EditorUtility.DisplayDialog("The Full Layout was Spawned!",
                "IMPORTANT: Please read the instructions on the spawned Schema Layout " +
                "by selecting the 'Instruction Panel' in the scene view or you may have runtime errors.",
                "I Understand");
        }
        ...
        private void ValidateSchema()
        {
            ...
            if (UIDesignerManager.UIDesignerValidationResult.validationState == UIDesignerSchema.ValidationState.Valid) {
                Debug.Log("The UI Designer Schema is valid!");
            } else {
    ...
    public class UIDesignerSchemaDuplicator
    {
        public static UIDesignerSchema DuplicateSchema(UIDesignerSchema previousSchema, string path)
        {
            ...
            try {
                var newSchema = duplicator.Duplicate(previousSchema, path);
                return newSchema;
            }
            catch (Exception e) {
                Debug.LogError("Failed to duplicate schema.");
                Debug.LogError(e);
        ...
        private UIDesignerSchema Duplicate(UIDesignerSchema sourceSchema, string newSchemaPath)
        {
            ...
            Debug.Log($"Duplicating the {sourceSchema.name} UI Designer Schema to {newSchemaPath}.");

            var databaseResult = AssetDatabase.CopyAsset(previousSchemaPath, newSchemaPath);

            if (databaseResult == false) {
                Debug.LogError("The UI Designer Schema could not be duplicated.");
                return null;
            }
            ...
            for (int i = 0; i < schemaFields.Length; i++) {
                ...
                var sourceGameObject =  AssetDatabase.LoadAssetAtPath<GameObject>(AssetDatabase.GetAssetPath(sourceObj));
                if (sourceGameObject != null && schemaSourceToNewObjectsDictionary.TryGetValue(sourceGameObject, out var existingDuplicateGameobject)) {
                    if (existingDuplicateGameobject != null) {
                        
                        //The gameobject was already duplicated but without the object type.
                        var gameobject = existingDuplicateGameobject as GameObject;
                        var component = gameobject.GetComponent(fieldType);

                        //Add it in if it exists.
                        if (component != null) {
                            schemaSourceToNewObjectsDictionary.Add(sourceObj, component);
                            field.SetValue(m_NewUIDesignerSchema, schemaSourceToNewObjectsDictionary[sourceObj]);
                            continue;
                        }
                    }
                ...
                var duplicateObj = DupplicateAssetToPath(sourceObj, schemaAssetsFolderPath, field.FieldType, out var duplicateObjectPath);
                if(duplicateObj == null) {
                    Debug.LogError($"The object '{field.Name}' could not be duplicated. IMPORTANT: The created schema will be broken. Delete it, recompile the scripts and try again.");
                    continue;
                }
                ...
                // Also add the gameobject if the source object is a component.
                // That's because some scripts reference gameobject prefabs instead of component prefabs.
                if (duplicateObj is Component duplicateObjComponent && sourceObj is Component sourceObjComponent) {
                    var duplicateGameObject = AssetDatabase.LoadAssetAtPath<GameObject>(duplicateObjectPath);

                    schemaSourceToNewObjectsDictionary[sourceGameObject] = duplicateGameObject;
                }
                
                // Special case for certain scriptable objects.
                
                // For CategoryItemActionSets, duplicate the itemActionSets inside
                if (duplicateObj is CategoryItemActionSet duplicateCategoryItemActionSet && sourceObj is CategoryItemActionSet sourceCategoryItemActionSet) {

                    for (int j = 0; j < sourceCategoryItemActionSet.CategoryItemActionsArray.Length; j++) {
                        var sourceItemActionSet = sourceCategoryItemActionSet.CategoryItemActionsArray[j];
                        
                        if(schemaSourceToNewObjectsDictionary.TryGetValue(sourceItemActionSet, out var existingValue)) {
                            duplicateCategoryItemActionSet.CategoryItemActionsArray[j] = existingValue as ItemActionSet;
                            continue;
                        }

                        var duplicateItemActionSet = (ItemActionSet)DupplicateAssetToPath(sourceItemActionSet, schemaAssetsFolderPath, typeof(ItemActionSet), out var duplicateItemActionSetPath);
                        if(duplicateItemActionSet == null) {
                            // Failed to duplicate
                            continue;
                        }

                        //Set the duplicate object to the new schema.
                        duplicateCategoryItemActionSet.CategoryItemActionsArray[j] = duplicateItemActionSet;
                        schemaSourceToNewObjectsDictionary.Add(sourceItemActionSet, duplicateItemActionSet);
                    }
                }
                
                if (duplicateObj is CategoryItemViewSet duplicateCategoryItemViewSet && sourceObj is CategoryItemViewSet sourceCategoryItemViewSet) {

                    for (int j = 0; j < sourceCategoryItemViewSet.CategoriesItemViews.Length; j++) {
                        var categoriesItemView = sourceCategoryItemViewSet.CategoriesItemViews[j];
                        var sourceItemViewPrefab = categoriesItemView.ItemViewPrefab;
                        
                        if(schemaSourceToNewObjectsDictionary.TryGetValue(sourceItemViewPrefab, out var existingValue)) {
                            duplicateCategoryItemViewSet.CategoriesItemViews[j] = new CategoryItemViews(categoriesItemView.Category, existingValue as GameObject);
                            continue;
                        }

                        var duplicateItemViewPrefab = (GameObject)DupplicateAssetToPath(sourceItemViewPrefab, schemaAssetsFolderPath, typeof(GameObject), out var duplicateItemViewPath);
                        if(duplicateItemViewPrefab == null) {
                            Debug.LogError($"The object '{sourceItemViewPrefab}' could not be duplicated. IMPORTANT: The created schema will be broken. Delete it, recompile the scripts and try again.");
                            continue;
                        }
                        
                        var sourceItemViewGameObject =  AssetDatabase.LoadAssetAtPath<GameObject>(AssetDatabase.GetAssetPath(sourceItemViewPrefab));
                        var duplicateItemViewGameObject = AssetDatabase.LoadAssetAtPath<GameObject>(duplicateItemViewPath);

                        //Set the duplicate object to the new schema.
                        duplicateCategoryItemViewSet.CategoriesItemViews[j] = new CategoryItemViews(categoriesItemView.Category, duplicateItemViewGameObject);
                        schemaSourceToNewObjectsDictionary.Add(sourceItemViewGameObject, duplicateItemViewGameObject);
                    }
            ...
            foreach (var keyValuePair in schemaSourceToNewObjectsDictionary) {
                ...
                if (duplicateGameObject == null) {
                    if (duplicate is Component component) {
                        ...
                        //Duplicate is not a prefab. It is probably a scriptable object.

                        var scriptableObject = duplicate as ScriptableObject;
                        if (scriptableObject == null) {
                            // The object is not a component, a gameobject or a scriptable object.
                            continue;
                        }
                        
                        ReplaceObjectReferencesToNewObjects(scriptableObject, flags, schemaSourceToNewObjectsDictionary);
                ...
                for (int i = 0; i < components.Length; i++) {
                    ...
                    ReplaceObjectReferencesToNewObjects(component, flags, schemaSourceToNewObjectsDictionary);
                }
        ...
        private static Object DupplicateAssetToPath(Object sourceObj, string schemaAssetsFolderPath, Type objectType, out string path)
        {
            ...
            var duplicateObj = (Object)AssetDatabase.LoadAssetAtPath(newPath, objectType);

            return duplicateObj;
        }

        /// <summary>
        /// Scan and replace the object reference in the object by the new objects in the dictionary.
        /// </summary>
        /// <param name="objectToScan">The object to scan.</param>
        /// <param name="flags">The binding flags.</param>
        /// <param name="schemaSourceToNewObjectsDictionary">The dictionary for source object to new.</param>
        private static void ReplaceObjectReferencesToNewObjects(Object objectToScan, BindingFlags flags,
            Dictionary<Object, Object> schemaSourceToNewObjectsDictionary)
        {
            var scriptableObjectFields = objectToScan.GetType().GetFields(flags);
            foreach (var fieldInfo in scriptableObjectFields) {
                var fieldType = fieldInfo.FieldType;

                if (fieldType != typeof(Object) && fieldType.IsSubclassOf(typeof(Object)) == false) { continue; }

                var serializedObject = fieldInfo.GetValue(objectToScan) as Object;
                if (serializedObject == null ||
                    schemaSourceToNewObjectsDictionary.TryGetValue(serializedObject, out var duplicateMatch) ==
                    false) { continue; }

                //Found match, replace it.
                fieldInfo.SetValue(objectToScan, duplicateMatch);
            }

            Shared.Editor.Utility.EditorUtility.SetDirty(objectToScan);
        }
